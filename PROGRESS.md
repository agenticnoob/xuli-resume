...
- 当前源版本：`e090a8fe78ce368e761859eab80ed95e69c0cbcd`。
- 生成 125 天日志（最新 2026-09-20）和 227 项技能；第二次同步 `changed=0`。
...
- Navbar 保持 `fixed`，滚动后使用不透明灰绿纸张遮罩、底部分隔与淡出边缘，避免导航文字与页面正文重叠。
- 数据来源与隐私说明移到实践日志顶部；“可写进简历的一件事”改为“今日总结”。
- 删除 timeline 的 10 条截断，展示公开快照中的全部 123 条 timeline event；节点改为绿色状态点并保留横向滚动与 scroll snap。
- 桌面日期目录保持 sticky，日期字号与字重提高；目录内部自动定位不再调用可能带动整页的 `scrollIntoView()`。
- 切换日期时使用文章容器定位，桌面与移动端都停在导航栏下方的正文顶部，不再滚回页面顶端。
- 单日技能使用记录由宽表格改为响应式卡片网格，清晰区分技能名、类别和当日次数。
