...
- 订阅 `auth.json` 只属于专用私有自动化仓库 `project-content` Environment 的加密 `CODEX_AUTH_JSON`；由仅具 `Environments: write` 的独立 session-store GitHub App 在生成前验证回写、生成后保存刷新结果，公开仓库不得保存、接收或上传该会话。
