...
2. Create and install a GitHub App only on `agenticnoob/xuli-resume`. Give it `Contents: Read and write`, `Pull requests: Read and write`, and `Metadata: Read`. Add its ID and private key to the private automation repository as `RESUME_APP_ID` and `RESUME_APP_PRIVATE_KEY` secrets.
...
5. Enable **Allow auto-merge** on `agenticnoob/xuli-resume`. Protect `main` with the `Verify build / build` check and the Vercel preview/deployment check used by this repository. The workflow stops before push on local validation failure, and GitHub auto-merge waits for required remote checks.
