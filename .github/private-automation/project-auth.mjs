...
export function validateManagedAuth(raw) {
  ...
  if (
    ...
    !["access_token", "refresh_token", "id_token", "account_id"].every(
      (key) => typeof auth.tokens?.[key] === "string" && auth.tokens[key].trim().length > 0,
    )
  ) {
...
async function readAuth(filename) {
  ...
  if (!info.isFile() || info.size > 40_000) throw new Error("Invalid Codex authentication file.");
...
export async function restoreAuth(root, raw) {
  ...
  await writeFile(path.join(root, "auth-before.json"), raw, { mode: 0o600, flag: "wx" });
  await writeFile(path.join(codexHome, "auth.json"), raw, { mode: 0o600, flag: "wx" });
}
...
export async function persistAuth(root, repository, token, run = runSecretWrite) {
  ...
  const before = validateManagedAuth(await readAuth(path.join(root, "auth-before.json")));
  ...
  const args = ["secret", "set", authSecret, "--repo", repository, "--env", authEnvironment];
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      ...
      if (attempt === 2) {
        throw new Error("Cannot save the Codex login; generation must not be published. Reseed the dedicated login before retrying.");
      }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  try {
    if (!directory || !["restore", "persist"].includes(command)) {
      throw new Error("Usage: project-auth.mjs restore|persist <temporary-directory>");
    }
    const root = path.resolve(directory);
    if (command === "restore") await restoreAuth(root, process.env.CODEX_AUTH_JSON);
    else await persistAuth(root, process.env.GITHUB_REPOSITORY, process.env.GH_TOKEN);
  } catch {
    console.error(`Project authentication ${command ?? "command"} failed. Check the private environment, dedicated login, and session-store App permission. No sensitive output is logged.`);
    process.exitCode = 1;
  }
}
